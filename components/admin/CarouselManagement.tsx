      ...
      <div className="grid gap-6">
        {carouselSlides.map((slide) => (
          <div key={slide.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden flex flex-col md:flex-row">
            <div className={`h-32 md:h-auto md:w-48 relative shrink-0 ${slide.color ? `bg-gradient-to-r ${slide.color}` : 'bg-gray-100 dark:bg-slate-700'}`}>
              <img
                src={slide.image}
                alt=""
                loading="lazy"
                className={`w-full h-full object-cover ${slide.color ? 'mix-blend-overlay opacity-50' : ''}`}
